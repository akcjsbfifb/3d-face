...
/**
 * Same framing as the original static index.html demo:
 * - SplatMesh from /face.splat
 * - quaternion (1,0,0,0) OpenCV→OpenGL
 * - position (0,0,-3), camera (0,0,3) looking at the splat
 * Mouse look is applied on top of that base orientation.
 */
export function FaceSplatCanvas({
  ...
  useEffect(() => {
    ...
    // Original demo: camera behind origin, orbit target on the splat
    ...
    const animate = () => {
      ...
      // Always keep the last frame when paused; only skip updates offscreen
    ...
    const init = async () => {
      try {
        ...
        // Same load path as the original index.html (proven working)
